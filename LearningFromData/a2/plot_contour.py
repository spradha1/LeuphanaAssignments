...
def plot_contour (X, y):
  ...
  plt.plot(whb[:,0], whb[:,1], color='g', linestyle='solid', linewidth=3, label='Batch')
  plt.plot(whs[:,0], whs[:,1], color='b', linestyle='solid', linewidth=3, alpha=0.5)
  plt.xlabel('$w_1$')
  plt.ylabel('$w_2$')
  ...
  wh_batch, _ = wh.widrow_hoff(X, y, 5000, True)
  wh_stoch, _ = wh.widrow_hoff(X, y, 5000, False)
  whb, whs = np.array(wh_batch), np.array(wh_stoch)

  plt.subplot(1, 2, 2)
  plt.contour(w0s, w1s, zs, 100, cmap='jet')
  plt.plot(whb[:,0], whb[:,1], color='g', linestyle='solid', linewidth=3, label='Batch')
  plt.plot(whs[:,0], whs[:,1], color='b', linestyle='solid', linewidth=3, label='Stochastic', alpha=0.5)
  plt.xlabel('$w_1$')
  plt.ylabel('$w_2$')
